...
def send_email(pdf_path: str, articles: list):
    ...
    # Build plain text body (no HTML at all, guaranteed to render correctly)
    body = []
    body.append("=" * 50)
    body.append(f"Daily News Digest - {date_str}")
    body.append("=" * 50)
    body.append("")
    body.append(f"Top {len(articles)} longest articles from past 24 hours:")
    body.append("")
    for i, a in enumerate(articles, 1):
        body.append(f"{i}. {a['title'][:80]}")
        body.append(f"   Source: {a.get('source_name', 'Unknown')} | {a.get('content_length', 0):,} chars")
        body.append(f"   URL: {a['url']}")
        body.append("")
    body.append("-" * 50)
    body.append("Full articles are in the attached PDF.")
    body.append("Generated by GitHub Actions | Daily News Digest")
